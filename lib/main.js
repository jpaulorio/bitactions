...
module.exports.start = async () => {
    ...
    const filteredRepositories = repositories.filter(function(item) {
        return typeof item == 'string' && !ignoredRepositories.includes(item.toLowerCase());
    });
    ...
    for (let i = 0; i < filteredRepositories.length; i++) {
        ...
        const filteredWorkflows = workflows.filter(function(item) {
            return typeof item == 'string' && !ignoredWorkflows.includes(item.toLowerCase());
        });
